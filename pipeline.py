...
from costants import INFERENCE_GRAPH, TEST_PDF_PATH, TEXT_TEMP_FOLDER, TABLE_TEMP_FOLDER
...
def pipeline(pdf_path, inference_graph_path, thread_name=None):
	...
	pdf_name = os.path.splitext(path_to_pdf)[0] \
		.split("\\")[-1]
	...
	for pil_image in bw_pil_gen:
		...
		if not c_tables == []:
			table_paths, text_path = write_crops(
				...
				cropped_text=None,
				temp_table_path=TABLE_TEMP_FOLDER,
				temp_text_path=None,
				page_number=page_number
			)
			for table_path in table_paths:
				do_tesseract_on_tables(table_path, TABLE_TEMP_FOLDER)
			counter_table = 0
			table_name = 'table_pag_{pag_num}'.format(pag_num=page_number)
			if page_number == 0:
				if os.path.isfile(os.path.join(TABLE_TEMP_FOLDER, pdf_name, table_name + '.txt')):
					os.remove(os.path.join(TABLE_TEMP_FOLDER, pdf_name, table_name + '.txt'))
			for c_t in c_tables:
				do_ocr_to_text(
					pil_image=c_t,
					file_name=table_name,
					output_folder=os.path.join(TABLE_TEMP_FOLDER, pdf_name)
				)
		...
		if c_text is not None:
			table_paths, text_path = write_crops(
				...
				temp_text_path=TEXT_TEMP_FOLDER,
				page_number=page_number
			)
			text_name = 'text'
			if page_number == 0:
				if os.path.isfile(os.path.join(TEXT_TEMP_FOLDER, pdf_name, text_name + '.txt')):
					os.remove(os.path.join(TEXT_TEMP_FOLDER, pdf_name, text_name + '.txt'))
			do_ocr_to_text(
				pil_image=c_text,
				file_name=text_name,
				output_folder=os.path.join(TEXT_TEMP_FOLDER, pdf_name)
			)
			page_number += 1
